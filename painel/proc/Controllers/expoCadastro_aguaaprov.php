...
//estancia o chamada processo como os dados
   ...
   $colunax = '';
   $colunay = '';
   $tabela  =  isset($_GET['tabela'])? $_GET['tabela'] : 'cadastro_aguaaprov';
   ...
   $lista = $exec->lista(
                     'id,'.$colunax.
                     "cad_equipamento, date_format(cad_cadastro ,'%d-%m-%Y %H:%i:%s') AS cadastro", 
                     $tabela);
   ...
   //monta o excel
   $arquivo = 'Aguardando_aprovacao_' . date("Ymd-His") .'.xls';
